...
async function run() {
  ...
  const result = await fetchOpen5eListSnapshot({
    rootDir: config.open5eSpellsRootDir,
    listUrl: config.open5eSpellsUrl,
  ...
  console.info(
    `Saved ${result.pageCount} spell page(s) from Open5e into ${result.snapshotDir} (${result.totalCount} records reported).`
  );
}
